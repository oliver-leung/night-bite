...
public class InputController {
	...
	public void readInput() {
		...
		if (xbox != null) {
			...
			int up, left, right, down, shoot, invisible;
			if (player == 0) {
				up = Input.Keys.UP;
			...
			// Allow the blue ship to also take in mouse inputs
			if (player == 0) {
				forward = Gdx.input.getDeltaY() * -0.25f;
				turning = Gdx.input.getDeltaX() * -0.05f;
			}

			visible = !Gdx.input.isKeyPressed(invisible);
