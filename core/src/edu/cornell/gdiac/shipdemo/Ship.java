...
public class Ship implements Entity {
	...
	public void setDiameter(int shipSize) {
    ...
    public void drawShip(GameCanvas canvas) {
        ...
        // Set a scale factor that matches the collision size.
		float scale = (float) (shipSize / 81.0);

        // Need to negate y scale because of coordinate access flip.
        // Draw the shadow first
		canvas.draw(shipSprite, stint, ox, oy, sx, sy, rotate, scale, scale);
		// Then draw the ship
		canvas.draw(shipSprite, tint, ox, oy, pos.x, pos.y, rotate, scale, scale);
	}
